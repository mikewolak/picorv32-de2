# Quartus II 8.0 Build Script - RISC-V 4-MIF Debug System
...
if {![project_exists riscv_4mif_debug]} {
    project_new riscv_4mif_debug
} else {
    project_open riscv_4mif_debug
}

# Device settings for DE-2 board
set_global_assignment -name FAMILY "Cyclone II"
set_global_assignment -name DEVICE EP2C35F672C6
set_global_assignment -name TOP_LEVEL_ENTITY test_4mif_riscv_debug

# Enable parallel compilation
set_global_assignment -name NUM_PARALLEL_PROCESSORS ALL

# VHDL source files
set_global_assignment -name VHDL_FILE ../../rtl/test_4mif_riscv_debug.vhd
...
# 4-Symbol Memory initialization files (same as memory test)
...
# Red LEDs for Knight Rider pattern (driven by firmware)
...
# Green LEDs for debug status indicators
...
# 7-segment displays (Program Counter)
...
# Try to compile
puts "Starting RISC-V 4-MIF Debug System compilation..."
if {[catch {execute_flow -compile} result]} {
    puts "Compilation failed: $result"
    project_close
    exit 1
} else {
    puts "RISC-V 4-MIF Debug System compilation successful!"
    project_close
}