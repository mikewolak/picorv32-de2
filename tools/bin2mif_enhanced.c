...
typedef struct {
    ...
    int four_symbol;         // Generate 4 symbol MIF files (byte 0-3 separation)
} config_t;
...
void print_usage(const char *program_name) {
    ...
    printf("      --4-symbol           Generate 4 symbol MIF files (byte 0-3 separation for 4-array memory)\n");
...
// Generate 4-symbol MIF files (byte 0-3 separation)
int generate_4_symbol_mif(config_t *config) {
    ...
    if (config->verbose) {
        printf("Generating 4-symbol MIF files from %ld byte firmware\n", file_size);
    }

    // Calculate memory size (default 32KB = 8192 words)
    int memory_words = config->total_size / 4;
    if (memory_words == 0) memory_words = 8192; // Default 32KB

    // Create 4 symbol files
    FILE *symbol_files[4];
    char symbol_filenames[4][MAX_FILENAME];

    for (int i = 0; i < 4; i++) {
        snprintf(symbol_filenames[i], MAX_FILENAME, config->output_pattern, i);
        symbol_files[i] = fopen(symbol_filenames[i], "w");
        if (!symbol_files[i]) {
            fprintf(stderr, "Error: Cannot create symbol file '%s'\n", symbol_filenames[i]);
            fclose(input_file);
            // Close previously opened files
            for (int j = 0; j < i; j++) {
                fclose(symbol_files[j]);
            }
            return -1;
        }

        // Write MIF header
        fprintf(symbol_files[i], "-- Generated 4-symbol MIF file (symbol %d)\n", i);
        fprintf(symbol_files[i], "-- Input: %s\n", config->input_file);
        fprintf(symbol_files[i], "-- Memory size: %d words (8-bit)\n", memory_words);
        fprintf(symbol_files[i], "\n");
        fprintf(symbol_files[i], "WIDTH=8;\n");
        fprintf(symbol_files[i], "DEPTH=%d;\n", memory_words);
        fprintf(symbol_files[i], "\n");
        fprintf(symbol_files[i], "ADDRESS_RADIX=HEX;\n");
        fprintf(symbol_files[i], "DATA_RADIX=HEX;\n");
        fprintf(symbol_files[i], "\n");
        fprintf(symbol_files[i], "CONTENT BEGIN\n");
    }

    // Read and split binary data
    for (int word_addr = 0; word_addr < memory_words; word_addr++) {
        uint32_t word_data = 0x00000013; // Default NOP

        // Read 32-bit word if within file
        if ((word_addr * 4) < file_size) {
            size_t bytes_read = fread(&word_data, 1, 4, input_file);
            if (bytes_read < 4) {
                // Pad partial words with zeros
                word_data &= (0xFFFFFFFF >> ((4 - bytes_read) * 8));
            }
        }

        // Extract bytes and write to symbol files
        uint8_t byte0 = (word_data >>  0) & 0xFF;
        uint8_t byte1 = (word_data >>  8) & 0xFF;
        uint8_t byte2 = (word_data >> 16) & 0xFF;
        uint8_t byte3 = (word_data >> 24) & 0xFF;

        fprintf(symbol_files[0], "%04X : %02X;\n", word_addr, byte0);
        fprintf(symbol_files[1], "%04X : %02X;\n", word_addr, byte1);
        fprintf(symbol_files[2], "%04X : %02X;\n", word_addr, byte2);
        fprintf(symbol_files[3], "%04X : %02X;\n", word_addr, byte3);
    }

    // Close MIF files
    for (int i = 0; i < 4; i++) {
        fprintf(symbol_files[i], "END;\n");
        fclose(symbol_files[i]);
    ...
    printf("Successfully generated 4-symbol MIF files:\n");
    for (int i = 0; i < 4; i++) {
        printf("  Symbol %d: %s\n", i, symbol_filenames[i]);
    }
...
int main(int argc, char *argv[]) {
    ...
    // Handle 4-symbol generation
    if (config.four_symbol) {
        return generate_4_symbol_mif(&config);
    }
